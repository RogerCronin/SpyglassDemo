...
async function addToGoal(req, res) {
    ...
    try {
        ...
        if(balance === null || balance === undefined) throw new Error("Error in bank statement")
        let goalAmounts = (await db.all("select current_amount from goal where user_id = ?", userID)).reduce((a, c) => a + c.current_amount, 0)
        if(balance - (goalAmounts + body.value) < 0) throw new Error("Not enough money in bank account")
        if(body.value <= 0) throw new Error("Invalid amount")
